...
# Iterative Stratification on mask_label and subject
X = data_csv["img_name"]
y = data_csv[["mask_label", "subject"]]

ikf = IterativeStratification(n_splits=5, order=1)

for index, (train_index, test_index) in enumerate(ikf.split(X, y)):
    train_df = data_csv.iloc[train_index]
    test_df = data_csv.iloc[test_index]

    #split train into train and val using the merged y
    train_df, val_df = train_test_split(train_df, test_size=0.2, random_state=42, stratify=train_df["mask_label"])
